...
- Hecho 2026-07-17 tarde (3): restauración COMPLETA. Netlify actualizado por API; Render recreado por API (`srv-d9d1m7r7uimc73fbro9g`, URL nueva `pokecollect-backend-t5ia.onrender.com`); 8 errores TS de admin arreglados en código (`users.is_admin`, `stripe_customer_id`) sin migración; NODE_VERSION 22 en Netlify. Deploy verde verificado (bundle solo con el proyecto nuevo; backend 200).
- Pendiente: smoke test del billing end-to-end y Billing Portal de Stripe (paso 6 RESTORE.md). Stripe en modo test clásico, 3 precios activos verificados.
